...
def index():
    ...
    study_winners = [
        Winner(spots=study_best_reviews[0] if study_best_reviews else "N/A"),
        Winner(spots=study_quietest[0] if study_quietest else "N/A"),
        Winner(spots=study_comfiest[0] if study_comfiest else "N/A"),
    ]
    grub_winners = [
        Winner(spots=grub_best_reviews[0] if grub_best_reviews else "N/A"),
        Winner(spots=grub_best_value[0] if grub_best_value else "N/A"),
        Winner(spots=grub_best_service[0] if grub_best_service else "N/A"),
    ]
    chill_winners = [
        Winner(spots=chill_best_reviews[0] if chill_best_reviews else "N/A"),
        Winner(spots=chill_coziest[0] if chill_coziest else "N/A"),
        Winner(spots=chill_least_crowded[0] if chill_least_crowded else "N/A"),
    ]
    shop_winners = [
        Winner(spots=shop_best_reviews[0] if shop_best_reviews else "N/A"),
        Winner(spots=shop_most_accessible[0] if shop_most_accessible else "N/A"),
        Winner(spots=shop_cleanest[0] if shop_cleanest else "N/A"),
    ]
